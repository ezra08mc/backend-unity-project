...
func ensureArray(resp []dto.TodoResponse) []dto.TodoResponse {
    if resp == nil {
...
func (t *TodoController) Create(ctx *gin.Context) {
	...
	if err := ctx.ShouldBindJSON(&p); err != nil {
		HandlerError(ctx, errs.BadRequest("Invalid payload"))
		return
	}
...
func (t *TodoController) GetActive(ctx *gin.Context) {
	...
	ctx.JSON(http.StatusOK, ensureArray(resp))
}
...
func (t *TodoController) SoftDelete(ctx *gin.Context) {
	userID, _ := getUserID(ctx)
	id, _ := strconv.Atoi(ctx.Param("id"))
	t.service.SoftDeleteByUserID(id, userID)
...
func (t *TodoController) Restore(ctx *gin.Context) {
	userID, _ := getUserID(ctx)
	id, _ := strconv.Atoi(ctx.Param("id"))
	t.service.RestoreByUserID(id, userID)
	ctx.JSON(http.StatusOK, gin.H{"success": true})
}
...
func (t *TodoController) PermanentDelete(ctx *gin.Context) {
	userID, _ := getUserID(ctx)
	id, _ := strconv.Atoi(ctx.Param("id"))
	t.service.PermanentDeleteByUserID(id, userID)
	ctx.JSON(http.StatusOK, gin.H{"success": true})
}
